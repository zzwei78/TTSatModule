...
static esp_err_t bq27220_i2c_read(i2c_master_dev_handle_t handle,
                                 uint8_t reg,
                                 size_t len,
                                 uint8_t *data)
{
...
{
    bq27220_data_t *bq_data = (bq27220_data_t *)bq_handle;
    esp_err_t res = bq27220_i2c_write(bq_data->i2c_device_handle, COMMAND_CONTROL, 2, (uint8_t *)&control);
    PRINT_ERROR(res);
    return res;
}
...
esp_err_t bq27220_set_parameter_u16(bq27220_handle_t bq_handle, uint16_t address, uint16_t value)
{
    ...
    buffer[3] = value & 0xFF;
    res = bq27220_i2c_write(bq_data->i2c_device_handle, COMMAND_SELECT_SUBCLASS, 4, buffer);
    PRINT_ERROR(res);
    delay_ms(10);
    ...
    buffer[0] = checksum;
    buffer[1] = 6;
    res = bq27220_i2c_write(bq_data->i2c_device_handle, COMMAND_MAC_DATA_SUM, 2, buffer);
    PRINT_ERROR(res);
    delay_ms(10);
    return res;
}
...
uint16_t bq27220_get_parameter_u16(bq27220_handle_t bq_handle, uint16_t address)
{
    ...
    buffer[0] = address & 0xFF;
    buffer[1] = (address >> 8) & 0xFF;
    res = bq27220_i2c_write(bq_data->i2c_device_handle, COMMAND_SELECT_SUBCLASS, 2, buffer);
    PRINT_ERROR(res);
    delay_ms(10);

    res = bq27220_i2c_read(bq_data->i2c_device_handle, COMMAND_MAC_DATA, 2, buffer);
    PRINT_ERROR(res);
...
bq27220_handle_t bq27220_create(const bq27220_config_t *config)
{
    ...
    if (i2c_master_bus_add_device(config->i2c_bus, &dev_config, &handle->i2c_device_handle) != ESP_OK) {
        ESP_LOGE(TAG, "i2c bus add device failed");
        return NULL;
    }
