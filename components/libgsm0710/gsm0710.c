...
{
    if (ctx != NULL) {
        /* frame_buffer is now static array, no need to free */
        free( ctx );
    }
}
...
gsm0710_packet( struct gsm0710_context *ctx, int channel, int type, const char *data, int len )
{
    ...
    else if ( type == GSM0710_STATUS_ACK && channel == 0 )
    {
        ...
        if ( len >= GSM0710_MIN_PACKET_LEN )
        {
            ...
            channel = original_channel;  // Restore channel for response
        }
...
void gsm0710_ready_read( struct gsm0710_context *ctx )
{
    ...
    /* Check if buffer is almost full - if so, we may have corrupt/incomplete frames */
    /* Discard old data to make room for new data */
    int available_space = sizeof( ctx->buffer ) - ctx->buffer_used;
    if ( available_space < 256 )
    {
        /*
         * Buffer almost full - this means we have incomplete frame(s) that couldn't be parsed.
         * Strategy: If buffer is > 50% full with unparseable data, flush it entirely.
         * This prevents the "stuck half-frame" problem where incomplete frames keep
         * accumulating and blocking new data.
         */
        if ( ctx->buffer_used > (int)(sizeof( ctx->buffer ) / 2) )
        {
            gsm0710_debug( ctx, "Buffer >50% full with incomplete frames, flushing entire buffer" );
            ctx->buffer_used = 0;
        }
        available_space = sizeof( ctx->buffer ) - ctx->buffer_used;
    }

    int len = ( *( ctx->read ) ) ( ctx, ctx->buffer + ctx->buffer_used, available_space );
    ...
    while ( posn < ctx->buffer_used )
    {
        if ( ctx->buffer[posn] == ( char )0xF9 )
        {
            ...
            /* Note: Tiantong module uses fixed FCS = 0x00, skip CRC check */
            /* Standard GSM0710 would verify CRC here, but Tiantong doesn't use it */
...
{
    /* Use pre-allocated frame buffer from context instead of stack allocation */
    char *frame = ctx->frame_buffer;
    int size;

    /* Lock mutex to protect frame_buffer from concurrent access */
    if ( ctx->lock )
        ( *( ctx->lock ) ) ( ctx );
