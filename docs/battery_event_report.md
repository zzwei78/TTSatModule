...
事件推送是**设备主动发送**的 Notify（无需 APP 请求），固定 **8 字节**：
...
7     charging     uint8    充电状态 (1=充电中, 0=未充电)
```
...
| 0x02 | `BATTERY_EVENT_CHARGE_START` | 充电开始 | — |
...
| 0x07 | `BATTERY_EVENT_PERIODIC` | 每 2 分钟固定推送（心跳） | — |
...
```
第 1 级：IP5561 充电标志（权威）
  power_manage_get_charging_status(&flags) == ESP_OK?
  → charging = (flags & 0x04) != 0   (bit2: charging active)
  → IP5561 是充电管理 IC，直接知道是否在充电

第 2 级：燃料计电流方向（fallback）
  IP5561 不可用时（未初始化/通信失败）
  → charging = (current < -200mA)    (统一 API: 负=充电)
  → 通过电流方向间接判断

状态转换:
  放电 → 充电               : 推送 CHARGE_START (0x02), charging=1
  充电 → 放电 (SOC ≥ 99%)   : 推送 CHARGE_FULL (0x04), charging=0
  充电 → 放电 (SOC < 99%)   : 推送 CHARGE_STOP (0x03), charging=0
```

### PERIODIC 心跳事件

每 2 分钟（12 × 10s 检测周期）自动推送一次 `BATTERY_EVENT_PERIODIC (0x07)`，包含当前完整电池快照。

用途：
- 防止 APP 因 BLE 连接不稳定漏收事件
- 作为设备存活心跳
- APP 定期同步电池状态

APP 收到 PERIODIC 时不需要特殊处理，直接用数据刷新界面即可。
...
| 周期心跳间隔 | 120 秒 (12 × 10s) | power_manage.c `BATT_PERIODIC_INTERVAL` |
| 默认 SOC 阈值 | 1% | gatt_system_server.h `BATTERY_REPORT_DEFAULT_SOC_THRESHOLD` |
| 充电判断电流阈值 (fallback) | 200 mA | power_manage.c `BATT_CHARGE_CURRENT_MA` |
...
设备推送 (Notify):
```
[0x7E][0x02][0x35][0x0C 0x10][0xBA 0x0B][0x01]

解析:
  0x7E          电池事件标识
  0x02          CHARGE_START
  0x35=53       SOC=53%
  0x100C=4108   电压=4108mV
  0x0BBA=3002   温度=300.2°K → 27.2°C
  0x01          charging=1（正在充电）
```
...
设备推送:
```
[0x7E][0x07][0x63][0x68 0x10][0xBB 0x0B][0x00]

解析:
  0x7E          电池事件标识
  0x07          PERIODIC（心跳）
  0x63=99       SOC=99%
  0x1068=4200   电压=4200mV
  0x0BBB=3003   温度=27.3°C
  0x00          charging=0（未充电）
```
...
| 文件 | 改动 |
|------|------|
| `main/ble/gatt_system_server.h` | 命令码 0x7E/0x7F + 事件类型常量(含 PERIODIC 0x07) + `send_battery_event()` 声明(含 charging 参数) |
| `main/ble/gatt_system_server.c` | `send_battery_event()` 实现(8字节 notify) + 命令处理 case + debug 白名单 |
| `main/system/power_manage.h` | `set_battery_report()` / `disable_battery_report()` 声明 |
| `main/system/power_manage.c` | 事件检测逻辑 + IP5561优先充电检测 + 周期心跳 + 每 10s 挂载到 monitor task |
...
- 充电检测优先使用 IP5561，IP5561 不可用时 fallback 到电流方向
...
| SOC 变化 | 等待电池放电 ≥1% | 收到 SOC_CHANGE 事件，charging 字段正确 |
| 开始充电 | 插入 USB | ≤10s 内收到 CHARGE_START，charging=1 |
| 充满 | 充电至 100% | 收到 CHARGE_FULL (SOC≥99 且停止充电)，charging=0 |
| 停止充电 | 拔出 USB (SOC<99%) | 收到 CHARGE_STOP，charging=0 |
| 高温 | 加热电池至 ≥55°C | 收到 TEMP_HIGH |
| 高温解除 | 降温至 ≤52°C | 不重复推送；再加热至 55°C 再次推送 |
| 低温 | 冷却至 ≤-10°C | 收到 TEMP_LOW |
| 周期心跳 | 等待 2 分钟 | 收到 PERIODIC，包含当前完整快照 |
